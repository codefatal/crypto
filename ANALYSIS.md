...
OHLCV 수집 (REST)
      │
      ▼
BakktaIndicator (기술적 지표 계산)  ──── score < 55 → 조기 종료
      │
      ▼
...
3개 소스를 **병렬**(`asyncio.gather`)로 수집:

| 소스 | 인증 | 내용 |
|---|---|---|
| 네이버 뉴스 API | NAVER_CLIENT_ID/SECRET 필요 | 코인별 한국어 최신 뉴스 (쿼리당 20개) |
| CoinDesk RSS | 불필요 | 영문 글로벌 헤드라인 5개 |
| Decrypt RSS | 불필요 | 영문 글로벌 헤드라인 5개 |
| 공포·탐욕 지수 | 불필요 | alternative.me, 0~100 점수 + 레이블 |

**코인별 필터링**: `NewsContext.for_coin("BTC")` — 네이버 뉴스만 코인별 필터링, 글로벌 RSS와 공포·탐욕 지수는 모든 코인 공통 적용
...
```
📈 KRW-BTC — LONG
🔥 신뢰도: HIGH (82.5/100) | 📌 현재가: `145,234,000`
...
| 시나리오 | 트리거 |
|---|---|
| `send_signal()` | AI 신뢰도 HIGH인 매매 신호 발생 시 |
| `send_system_status()` | 봇 시작 / 종료 시 |
| `send_news_digest()` | 봇 시작 후 첫 뉴스 수집 완료 시 |
| `send_error()` | 시스템 에러 발생 시 |
...
5. send_system_status()        → Discord + Telegram 시작 알림
6. scanner.start()             → 블로킹 (무한 루프)
```
