...
| `send_breakout_alert()` | 규칙 기반 5/5 조건 충족 | 하늘색 embed (충족 조건 목록, 심볼 `KRW-BTC(비트코인)` 형식) |
| `send_spike_alert()` | 현재가 vs 기준가 ±10% 이상 | 주황/보라 embed (변동률, 현재가, 기준가) |
