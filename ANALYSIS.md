...
AIAnalyzer (Groq/Llama 3 AI 분석)
      ...
      ├─ 폴백(NEUTRAL) → DB 기록만
      │
      ├─ [BTC + MEDIUM/LOW] → send_signal_brief() (간단 알림, 거래 없음)
      │
      ▼ [confidence=HIGH만]
...
[봇 시작 시] → _analyze_initial_coins() → BTC/ETH 즉시 분석 + 알림
```
...
| 시나리오 | 트리거 | 포맷 |
|---|---|---|
| `send_signal()` | AI 신뢰도 HIGH인 매매 신호 | 전체 embed (신뢰도·현재가·손절·익절·근거·리스크) |
| `send_signal_brief()` | BTC MEDIUM/LOW 신호 (거래 없음) | 간단 embed (신뢰도·현재가·판단근거만) |
...
7. _analyze_initial_coins() 생성    → BTC/ETH 즉시 분석 + 알림 (upbit 전용)
8. scanner.start()                  → 블로킹 (무한 루프)
```
  ...
  └─ score ≥ 70.0
       ...
       └─ AIAnalyzer.analyze(symbol, result, news_ctx)
              ...
              └─ is_fallback=False
                   ...
                   └─ is_btc=True + MEDIUM/LOW
                        └─ Notifier.send_signal_brief(decision)  (거래 없음)
```
